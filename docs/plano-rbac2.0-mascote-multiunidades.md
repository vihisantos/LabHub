...
### FASE 8.2 — UI DE GESTÃO DO COORDENADOR (sessão 2026-09-08)

Implementada sobre a fundação validada (047 no DEV, 36/36 adversarial). **Sem nova camada de autorização no frontend**: a UI replica visualmente o conjunto de gestores que o RPC aceita e deixa o Postgres (RPC 047 + guard 046) validar. **Nenhuma alteração em `App.tsx` nem nas Fases 4–7** (rota `/coordenador` e `LeadershipAreaGuard` seguem pendentes de integração — fora deste commit).

- [x] **Atribuição de membro a gestor** — botão `+` no membro abre um sheet com radiogrupo de gestores **apenas os permitidos pela árvore/unidade** (`managerOptionsForMember`): a membership de coordenação da unidade OU uma liderança já subordinada direta a ela; exclui o gestor atual e o próprio membro. Confirma chamando `setCoordinatorManager(membershipId, managerId)`.
- [x] **Remoção de membro da equipe** — botão `−` chama `setCoordinatorManager(membershipId, null)` (gestor NULL = remove; guard 046 sempre permite). **Refresh da árvore após sucesso** (vazio honesto e legítimo).
- [x] **Anti-duplicação** — uma única `pending` ativa desabilita **todas** as ações de gestão (vincular/remover) enquanto a escrita está em andamento; o botão de confirmação mostra `Vinculando...`.
- [x] **Tratamento de erro** — erro do RPC na atribuição fica **inline no sheet** (que não fecha), com `getLastCoordinatorServiceError()`; erro na remoção vira **banner** descartável. Nenhum refresh falso em falha.
- [x] **Testes (8 novos na tela; 14 totais no arquivo)** — `CoordinatorHome.test.tsx` reescrito: candidatos restringidos (coordenação + lideranças da unidade, exclui gestor atual/self), atribuição (RPC + refresh + fecha sheet), remoção (RPC + refresh), erro do RPC no assign (inline, sem refresh), erro no remove (banner), anti-duplicação (desabilitado durante escrita), sem CTA de criar membership/alterar cargo.
- [x] **Bateria 8.2:** **8/8** testes específicos verdes. **Suíte global: 203 arquivos · 1767 passed · 2 failed · 1 skip** — as 2 falhas são **pré-existentes e fora da 8.2**: `AuthGuard.test.tsx` (PR #167 merged) asserta a waiting area do HEAD (tela inline), mas a working tree WIP das Fases 4–7 trocou por `<Navigate to="/approval-pending">` sem Router no teste → `<Navigate> may be used only in the context of a <Router>`. Lint exit 0 (warnings pré-existentes + 1 novo `only-export-components` em `CoordinatorHome`, mesmo padrão já aceito em `LoginPage`/`Onboarding`); build `tsc -b && vite build` verde.

**Próximos passos da Fase 8:** (a) **higienização/compatibilidade AuthGuard** — etapa separada (fora da 8.2): provável `MemoryRouter` nos testes do PR #167 contra o WIP local das Fases 4–7, preservando a separação; (b) fechar o drift `workspace_ids` (legado) ↔ memberships como fonte de escopo; (c) auditoria de fechamento da Fase 8 (espelho 7.7). O mascote permanece congelado.