...
export function StockQRScanner() {
  ...
  const navigateToItem = useCallback((code: string) => {
    const item = findStockItemByQrCode(items, code)
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-surface text-fg">
      <header className="flex items-center gap-2 border-b border-line bg-card/80 px-3 py-2.5 backdrop-blur-xl">
        ...
        <h1 className="text-sm font-semibold text-fg">Escanear QR — Estoque</h1>
      </header>
      ...
      <div className="flex flex-1 flex-col items-center px-5 pb-8 pt-4">
        ...
        <form onSubmit={handleManualSubmit} className="flex w-full max-w-sm gap-2">
          <input
            ...
            placeholder="Ex: maquinas/Notebook Dell"
        ...
        {feedback === 'error' && (
          <p className="mt-3 text-xs text-red-400">Nenhum item encontrado com esse código</p>
        )}

        <div className="mt-4 w-full max-w-sm rounded-xl bg-card/50 p-3 text-center text-[11px] text-fg-dim leading-relaxed">
          Escaneie um QR code gerado pelo LabHub para localizar o item no estoque.
          <br />
          Formato: <code className="rounded bg-input px-1 font-mono text-fg-muted">seção/nome</code>
        </div>
      </div>
    </div>
  )
