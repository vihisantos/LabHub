          ...
          return (
            ...
            >
              <div style={{ borderBottom: '1px solid var(--border)', paddingBottom: '10px', marginBottom: '12px' }}>
                <div style={{ fontSize: '11px', fontWeight: 800, color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '2px' }}>
                  {diaSemana}
                </div>
                <div style={{ fontSize: '24px', fontWeight: 900, color: 'var(--text-primary)', display: 'flex', alignItems: 'baseline', gap: '2px', lineHeight: 1 }}>
                  {dia}
                  <span style={{ fontSize: '14px', fontWeight: 600, color: 'var(--text-muted)' }}>/{String(mes).padStart(2, '0')}</span>
                </div>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {day.reservations.slice(0, 3).map((r, j) => (
                  <div key={j} style={{ fontSize: '12px', padding: '8px', borderRadius: '4px', background: 'var(--bg-input)' }}>
                    <div style={{ fontWeight: 600, color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: '6px' }}>
                      <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: r.tipo === 'tablet' ? 'var(--accent)' : 'var(--text-primary)', flexShrink: 0 }} />
                      {r.lab}
                    </div>
                    <div style={{ color: 'var(--text-muted)' }}>{r.time}</div>
                  </div>
                ))}
                {day.reservations.length === 0 && (
                  <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>0 reservas</div>
                )}
          ...
          >
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem' }}>
                <div>
                  <h2 style={{ fontSize: '1.5rem', fontWeight: 700, color: 'var(--text-primary)' }}>{selectedDay.date}</h2>
                  <p style={{ fontSize: '14px', color: 'var(--accent)', textTransform: 'uppercase' }}>{selectedDay.dayName}</p>
                </div>
                <button onClick={() => setSelectedDay(null)} style={{ padding: '10px', borderRadius: '50%', border: 'none', background: 'var(--bg-input)', cursor: 'pointer', minHeight: '44px', minWidth: '44px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                ...
                {[
                  ...
                  <button
                    ...
                    style={{
                      ...
                      background: filtroTipo === t.id ? 'var(--accent)' : 'var(--bg-input)',
                      color: filtroTipo === t.id ? '#ffffff' : 'var(--text-secondary)',
                    }}
                  >
                    {t.label}
                  </button>
                ...
                {periodos.map((p) => (
                  <button
                    ...
                    style={{
                      ...
                      background: filtroPeriodo === p.id ? 'var(--text-primary)' : 'var(--bg-input)',
                      color: filtroPeriodo === p.id ? 'var(--bg-card)' : 'var(--text-secondary)',
                    }}
                  >
                    {p.label}
                  </button>
              ...
              {/* Legend */}
              <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem', padding: '1rem', background: 'var(--bg-input)', borderRadius: '0.5rem', flexWrap: 'wrap' }}>
                <LegendItem color="var(--text-primary)" label="Lab" />
                <LegendItem color="var(--accent)" label="Tablet" />
                ...
                <AnimatePresence>
                  {reservasFiltradas.map((r, i) => (
                    <motion.div
                      ...
                      style={{ padding: '1rem', borderRadius: '0.75rem', border: '1px solid var(--border)', background: 'var(--bg-card)' }}
                    >
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          <span style={{ width: '8px', height: '8px', borderRadius: '2px', background: r.tipo === 'tablet' ? 'var(--accent)' : 'var(--text-primary)', display: 'block' }} />
                          <span style={{ fontWeight: 600, color: 'var(--text-primary)', fontSize: '14px' }}>{r.lab}</span>
                          {r.tipo === 'tablet' && (
                            <span style={{ fontSize: '10px', fontWeight: 600, padding: '2px 6px', borderRadius: '4px', background: 'var(--accent-soft)', color: 'var(--accent)' }}>
                              Tablet
                            </span>
                          )}
                        </div>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '0.5rem' }}>
                        <span style={{ color: 'var(--text-muted)', display: 'flex' }}><User size={14} /></span>
                        <span style={{ fontSize: '13px', color: 'var(--text-muted)' }}>Professor:</span>
                        <span style={{ fontSize: '13px', color: 'var(--text-primary)', fontWeight: 500 }}>{r.professor || '—'}</span>
                      </div>

                      {r.reservaFeitaPor && (
                        <div style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', marginBottom: '0.5rem' }}>
                          <span style={{ color: 'var(--text-muted)', marginTop: '2px', display: 'flex' }}><User size={14} /></span>
                          <div>
                            <span style={{ fontSize: '13px', color: 'var(--text-muted)' }}>Reservado por:</span>
                            <span style={{ fontSize: '13px', color: 'var(--text-primary)', fontWeight: 500, marginLeft: '4px' }}>{r.reservaFeitaPor}</span>
                          </div>
                        </div>
                      )}

                      {r.observacao && (
                        <div style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', paddingTop: '0.5rem', borderTop: '1px solid var(--border)' }}>
                          <span style={{ color: 'var(--text-muted)', marginTop: '2px', display: 'flex' }}><BookOpen size={14} /></span>
                          <div>
                            <span style={{ fontSize: '13px', color: 'var(--text-muted)' }}>{r.tipo === 'tablet' ? 'Finalidade:' : 'Disciplina:'}</span>
                            <span style={{ fontSize: '13px', color: 'var(--text-primary)', fontWeight: 500, marginLeft: '4px' }}>{r.observacao}</span>
                          </div>
                        </div>
                      )}
                      <button
