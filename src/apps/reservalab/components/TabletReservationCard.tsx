...
function getStatus(res: TabletReserva) {
  ...
  if (agora > fim) return { label: 'ENCERRADA', color: 'var(--text-muted)', bg: 'rgba(148,163,184,0.12)' }
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        padding: '1.25rem',
        borderRadius: '1rem',
        border: isLive ? '1px solid rgba(239,68,68,0.5)' : '1px solid var(--border)',
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
        <h4 style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--text-primary)' }}>{reservation.sala}</h4>
      ...
      {reservation.reservado_por && (
        <p style={{ fontSize: '0.7rem', color: 'var(--text-muted)', marginTop: '0.5rem', borderTop: '1px solid var(--border)', paddingTop: '0.4rem' }}>
